...
async function checkLoginPersistence() {
    ...
    try {
        ...
        if (isValid) {
            ...
            const storedCollapse = localStorage.getItem(`collapsed_tasks_${state.user.name}`);
            if (storedCollapse) {
                try {
            ...
            // 🌟 CRITICAL FIX: Await the data sync stream before letting the app render
            await refreshTasks();
        } else {
...
function initThemeManager() {
    const config = window.APP_CONFIG; // ✅ Fixed missing declaration scope boundary
    const currentTheme = localStorage.getItem('app_theme') || (config ? config.defaultTheme : 'dark');

    // Apply initial configuration attribute signatures safely
    const targetTheme = currentTheme === 'auto'
        ? (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')
        : currentTheme;

    document.documentElement.setAttribute('data-theme', targetTheme);

    const toggleBtn = document.getElementById('theme-toggle'); // ✅ Fixed explicit typo naming mapping bounds
    if (toggleBtn) {
        toggleBtn.innerText = targetTheme === 'dark' ? '☀️ Light Mode' : '🌙 Dark Mode';
...
function createNavbarComponent() {
    ...
    nav.className = "navbar"; // ✅ Linked cleanly directly into your custom stylesheet
    ...
    taskTab.className = state.currentView === 'tasks' ? "nav-btn active" : "nav-btn";
    taskTab.innerHTML = "📋 Tasks";
    taskTab.onclick = () => { state.currentView = 'tasks'; renderView(); };

    // ⏳ Attendance Tab Setup
    const absenceTab = document.createElement('button');
    absenceTab.className = state.currentView === 'absence' ? "nav-btn active" : "nav-btn";
    absenceTab.innerHTML = "⏳ Attendance";
    absenceTab.onclick = () => { state.currentView = 'absence'; renderView(); };
    ...
    themeBtn.onclick = () => {
        ...
        themeBtn.innerText = nextTheme === 'dark' ? "☀️ Light" : "🌙 Dark";
    };
...
function createLoginComponent() {
    ...
    card.querySelector('#btn-login').onclick = async () => {
        ...
        try {
            ...
            errText.innerText = "Authenticating...";
            errText.style.display = 'block';
            errText.style.color = 'var(--text-secondary)';

            const csvUrl = window.APP_CONFIG.spreadsheetCsvLinks.users;
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const cleanRow = lines[i].split(',').map(item => item.replace(/^["']|["']$/g, '').trim());

                if (cleanRow[0] === userId && cleanRow[1] === pin) {
                    authenticatedUser = {
                        name: cleanRow[2],
                        team: cleanRow[3]
                    };
            ...
            if (authenticatedUser) {
                ...
                errText.innerText = "Invalid User ID or PIN.";
            ...
            card.querySelector('#btn-login').disabled = false;
            errText.innerText = "Network link execution failure.";
            errText.style.color = 'var(--warning)';
            errText.style.display = 'block';
            console.error(err);
        }
...
function createTasksComponent() {
    ...
    header.style.cssText = "margin-bottom: 20px; font-size: 16px; font-weight: 600; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.5px;";
    ...
    state.tasks.forEach((task, index) => {
        ...
        checkboxInput.onchange = (e) => {
            ...
            localStorage.setItem(`collapsed_tasks_${state.user.name}`, JSON.stringify(state.collapsedTasks));
            renderView();
        };
        ...
        cardHeader.onclick = (e) => {
            if (e.target.closest('.proof-checkbox-wrap') || e.target.type === 'checkbox') return;
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((pos) => {
            const distance = calculateHaversineDistance(
                ...
                window.APP_CONFIG.officeLocation.latitude, window.APP_CONFIG.officeLocation.longitude
            );

            if (distance <= window.APP_CONFIG.officeLocation.radiusMeters) {
                statusText.innerText = "Location Authorized ✅ Inside Office Perimeter.";
            ...
            statusText.innerText = "Hardware GPS Query Error. Lock execution barred.";
        }, { enableHighAccuracy: true });
    } else {
        statusText.innerText = "Geofencing Core modules unavailable on this architecture.";
    }
...
/**
 * ☁️ Data Sync Pipelines: Fetch tasks directly from the spreadsheet
 */
async function refreshTasks() {
    try {
        const csvUrl = window.APP_CONFIG.spreadsheetCsvLinks.tasks;
        const csvResponse = await fetch(csvUrl);
        const csvText = await csvResponse.text();
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const currentLineValues = parseCsvLine(line);
            const taskObj = {};

            headers.forEach((header, index) => {
                taskObj[header] = currentLineValues[index] !== undefined ? currentLineValues[index] : '';
            });

            fetchedTasks.push(taskObj);
        }

        state.tasks = fetchedTasks;

    } catch (e) {
        console.error("Task synchronizer runtime failure. Reverting to fallback.", e);
        state.tasks = [];
    }
}

/**
 * 📤 Data Sync Pipelines: Handle Base64 Binary Image streams transformations
 */
async function uploadWorkProof(index, clientName) {
    ...
    reader.onloadend = async () => {
        ...
        await fetch(window.APP_CONFIG.googleAppsScriptUrl, {
            method: 'POST',
            mode: 'no-cors',
            body: JSON.stringify(payload)
        });

        alert("Upload verification signature passed to gateway processing center.");
    };
    reader.readAsDataURL(file);
}

/**
 * 📤 Data Sync Pipelines: Submit attendance operations tracking timestamps
 */
async function submitAttendance(type) {
    ...
    alert(`Attendance protocol [Check-${type.toUpperCase()}] executed cleanly.`);
}